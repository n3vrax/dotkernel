...
class OAuth2ServerInstanceFactory
{
    ...
    public function __invoke()
    {
        ...
        if (isset($availableGrantTypes['authorization_code']) && $availableGrantTypes['authorization_code'] === true) {
            if(isset($options['use_openid_connect']) && $options['use_openid_connect'])
            {
                //this is the only modifications from the original one
                //it allows id_token to be given in code flow
